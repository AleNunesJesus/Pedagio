-- Reimportar o mesmo arquivo de passagens duas vezes criava linhas
-- duplicadas (passagem_pedagio não tinha nenhuma trava, diferente de
-- posicao_veiculo que já ganhou unique constraint na FASE 09). A chave
-- inclui condicao+valor_cobrado (não só placa+data_hora) porque débito e
-- crédito legitimamente pareados compartilham placa/horário — só o
-- valor/condição os diferencia.
--
-- Limitação conhecida (decisão adiada): uma fatura reemitida/corrigida
-- pelo fornecedor com o mesmo veículo+horário+condição+valor da original
-- seria silenciosamente ignorada nesta constraint. Revisitar se esse
-- cenário aparecer na prática.
alter table pedagio.passagem_pedagio
  add constraint passagem_pedagio_dedup_uniq unique (placa_informada, data_hora, condicao, valor_cobrado);
...
begin
  ...
  for v_row in select * from pedagio.staging_passagem_pedagio where id = any(v_ids) order by id loop
    ...
    if not found then
      -- linha duplicada (mesma placa+horário+condição+valor já importada antes)
      v_erros := v_erros + 1;
    end if;
  end loop;
